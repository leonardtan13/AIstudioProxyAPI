...
class PageController:
    ...
    async def _handle_thinking_budget(
        ...
        # 检查用户是否明确禁用了思考预算
        should_disable_budget = (
            isinstance(reasoning_effort, str) and reasoning_effort.lower() == "none"
        )

        if should_disable_budget:
            self.logger.info(
                f"[{self.req_id}] 用户通过 reasoning_effort='none' 明确禁用思考预算。"
            )
            ...
            self.logger.info(
                f"[{self.req_id}] 用户指定了 reasoning_effort: {reasoning_effort}，将启用并设置思考预算。"
            )
            await self._control_thinking_budget_toggle(
                should_be_checked=True,
                check_client_disconnected=check_client_disconnected,
            )
            await self._adjust_thinking_budget(
                reasoning_effort, check_client_disconnected
            )
        else:
            # 用户未指定，根据默认配置
            self.logger.info(
                f"[{self.req_id}] 用户未指定 reasoning_effort，根据默认配置 ENABLE_THINKING_BUDGET: {ENABLE_THINKING_BUDGET}。"
            )
            await self._control_thinking_budget_toggle(
                should_be_checked=ENABLE_THINKING_BUDGET,
                check_client_disconnected=check_client_disconnected,
            )
    ...
    def _parse_thinking_budget(self, reasoning_effort: Optional[Any]) -> Optional[int]:
        ...
        if reasoning_effort is None:
            ...
            self.logger.info(
                f"[{self.req_id}] 'reasoning_effort' 为空，使用默认思考预算: {token_budget}"
            )
            ...
            if reasoning_effort.lower() == "none":
                token_budget = DEFAULT_THINKING_BUDGET
                self.logger.info(
                    f"[{self.req_id}] 'reasoning_effort' 为 'none' 字符串，使用默认思考预算: {token_budget}"
                )
            else:
        ...
        if token_budget is None:
            self.logger.warning(
                f"[{self.req_id}] 无法从 '{reasoning_effort}' (类型: {type(reasoning_effort)}) 解析出有效的 token_budget。"
            )

        return token_budget

    async def _adjust_thinking_budget(
        self, reasoning_effort: Optional[Any], check_client_disconnected: Callable
    ):
        """根据 reasoning_effort 调整思考预算。"""
        self.logger.info(
            f"[{self.req_id}] 检查并调整思考预算，输入值: {reasoning_effort}"
        )
        ...
        if token_budget is None:
            self.logger.warning(
                f"[{self.req_id}] 无效的 reasoning_effort 值: '{reasoning_effort}'。跳过调整。"
            )
            return
        ...
        try:
            ...
            if int(new_value_str) == token_budget:
                ...
                self.logger.warning(
                    f"[{self.req_id}] ⚠️ 思考预算更新后验证失败。页面显示: {new_value_str}, 期望: {token_budget}"
                )
    ...
    async def _control_thinking_budget_toggle(
        ...
        try:
            ...
            if current_state_is_checked != should_be_checked:
                ...
                self.logger.info(
                    f"[{self.req_id}] 思考预算开关当前状态与期望不符，正在点击以{action}..."
                )
                await toggle_locator.click(timeout=CLICK_TIMEOUT_MS)
                await self._check_disconnect(
                ...
                self.logger.info(
                    f"[{self.req_id}] 'Thinking Budget' 开关已处于期望状态，无需操作。"
                )
    ...
    async def _navigate_via_chat_link(
        self, check_client_disconnected: Callable
    ) -> bool:
        """通过导航栏链接尝试重置聊天。"""
        nav_link_locator = self.page.locator(NAV_CHAT_LINK_SELECTOR)

        try:
            await expect_async(nav_link_locator).to_be_visible(timeout=2000)
            await self._check_disconnect(
                check_client_disconnected, "导航栏清空 - 链接可见后"
            )

            await nav_link_locator.click(timeout=CLICK_TIMEOUT_MS)
            await self._check_disconnect(
                check_client_disconnected, "导航栏清空 - 点击链接后"
            )

            await self.page.wait_for_url("**/prompts/new_chat", timeout=3000)
            await asyncio.sleep(0.3)

            self.logger.info(f'[{self.req_id}] 通过导航栏"Chat"链接完成聊天重置。')
            return True
        except TimeoutError as nav_timeout:
            self.logger.warning(
                f"[{self.req_id}] Chat navigation link reset unavailable (timeout): {nav_timeout}"
            )
        except Exception as nav_error:
            self.logger.warning(
                f"[{self.req_id}] Chat navigation link reset unavailable: {nav_error}"
            )
    ...
    async def clear_chat_history(self, check_client_disconnected: Callable):
        ...
        try:
            nav_reset_successful = await self._navigate_via_chat_link(
                check_client_disconnected
            )
            reset_completed = nav_reset_successful

            if nav_reset_successful:
                self.logger.info(
                    f'[{self.req_id}] 已通过导航栏"Chat"链接重置聊天，跳过弹窗清空流程。'
                )
            else:
                ...
                try:
                    ...
                    self.logger.info(
                        f"[{self.req_id}] 发送按钮可用，尝试点击并等待1秒..."
                    )
                ...
                clear_chat_button_locator = self.page.locator(
                    CLEAR_CHAT_BUTTON_SELECTOR
                )
                ...
                try:
                    ...
                    self.logger.info(
                        f'[{self.req_id}] "清空聊天"按钮可用，继续清空流程。'
                    )
            ...
            if reset_completed:
                self.logger.info(
                    f"[{self.req_id}] 聊天已重置，重新启用 '临时聊天' 模式..."
                )
            else:
                self.logger.info(
                    f"[{self.req_id}] 未能确认聊天已重置，仍尝试启用 '临时聊天' 模式..."
                )
    ...
    async def _dismiss_backdrops(self):
        ...
        try:
            ...
            for i in range(3):
                ...
                if cnt and cnt > 0:
                    self.logger.info(
                        f"[{self.req_id}] 检测到透明遮罩层 ({cnt})，发送 ESC 以关闭 (尝试 {i + 1}/3)。"
                    )
                    try:
        ...
        try:
            ...
            if not submitted_successfully:
                ...
                try:
                    ...
                    self.logger.error(
                        f"[{self.req_id}] ❌ 提交按钮点击失败: {click_err}"
                    )
                ...
                if not button_clicked:
                    ...
                    if not combo_ok:
                        ...
                        raise Exception(
                            "Submit failed: Enter, Button, and Combo key all failed"
                        )
